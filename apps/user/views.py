...
class LoginView(View):
    ...
    def get(self, request):
        ...
        # 使用模板
        print({'username': username, 'checked': checked})
        return render(request, 'login.html', {'username': username, 'checked': checked})

    def post(self, request):
        '''登录校验'''
        # 接收数据
        username = request.POST.get('username')  #input标签的name属性
        password = request.POST.get('pwd')
        ...
        if user is not None:
            ...
            if user.is_active:
                ...
                # 跳转到next_url
                response = redirect(next_url)  # HttpResponseRedirect  #用户验证成功后，先login，再跳转，再保存cookie

                # 判断是否需要记住用户名
                remember = request.POST.get('remember') #勾选了checkbox，值为on
                if remember == 'on':
                    # 记住用户名
